...
function UpdateStduent() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8080/student/${id}`).then((response) => {
            setName(response.data.studentName)
            setCollege(response.data.collegeName)
    ...
    return (
        <div className="bg-[#e0e4fd] justify-center flex flex-row ">
            <form className="flex flex-col  justify-center p-[20px] w-[500px] text-center bg-white my-10 rounded-md">
                <h3 className="text-[27px] uppercase font-bold text-[#3f51b5] mb-3">Update student</h3>
                <input type='text' placeholder="Student ID" className="mb-3 p-2  focus:outline-none border-2 border-[#6793e0] rounded-md focus:border-[#0008c5]" value={studentId} readOnly />
                <input type='text' placeholder="Student Name" className={`mb-3 p-2  focus:outline-none border-2 ${error.name ? 'border-red-700' : 'border-[#6793e0]'} rounded-md focus:border-[#0008c5]`} value={name} onChange={(e) => setName(e.target.value)} />
                ...
                <div className="text-center">
                    <Link to='/'> <button type="button" className="bg-red-500 py-2 px-4 text-[20px] font-semibold rounded-[4px] uppercase mt-3 mr-5 text-white hover:ring-blue-400 hoverbtn-shadows  hover:bg-[#f96060]">Cancel ✖️</button></Link>
                    <button type="button" className=" bg-[#0052d7] py-2 px-4 text-[20px] font-semibold rounded-[4px] uppercase mt-3 text-white hover:ring-blue-400 hoverbtn-shadows  hover:bg-blue-500" onClick={updateStduent}>Update ✏️</button>
                </div>
            </form>
        </div>
    );
